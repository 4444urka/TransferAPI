    ...
    def clean_trip_seats(self):
        ...
        if trip and trip.is_active == False and trip.is_bookable == False:
            raise ValidationError(
                "Выбранная поездка не активна или не доступна для бронирования."
            )
            ...
            for trip_seat in trip_seats:
                ...
                if trip_seat.is_booked:
                    ...
                    if self.instance and self.instance.pk:
                        current_booking_seat_ids = list(
                            self.instance.trip_seats.values_list("id", flat=True)
                        )
                        if trip_seat.id not in current_booking_seat_ids:
                            raise ValidationError(
                                f"Место {trip_seat.seat} уже забронировано другим пользователем."
                            )
                    else:
                        raise ValidationError(
        ...
        if payment and trip and trip_seats:
            ...
            for trip_seat in trip_seats:
                ...
                if seat.seat_class == "comfort":
                    expected_total_price += trip.comfort_seat_price
                elif seat.seat_class == "economy":
                    expected_total_price += trip.economy_seat_price
