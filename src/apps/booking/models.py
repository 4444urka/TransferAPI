...
class Booking(models.Model):
    ...
    trip = models.ForeignKey(
        Trip, on_delete=models.CASCADE, default=1, verbose_name="Поездка"
    )
    ...
    booking_datetime = models.DateTimeField(
        auto_now_add=True, verbose_name="Дата и время бронирования"
    )
    is_active = models.BooleanField(default=True, verbose_name="Бронирование актуально")
    ...
    def total_price(self):
        ...
        for trip_seat in self.trip_seats.all():
            ...
            if seat.seat_class == "comfort":
                price += self.trip.comfort_seat_price
            elif seat.seat_class == "economy":
                price += self.trip.economy_seat_price
        return price
    ...
    def __str__(self):
        ...
        return (
            f"{self.booking_datetime} - {self.user} - {self.trip} - {self.total_price}"
        )
    ...
    class Meta:
        ...
        ordering = [
            "-booking_datetime"
        ]  # Только сортировка по дате бронирования без фильтрации
    ...
    def clean(self):
        ...
        if self.pickup_location and self.pickup_location.strip():
            ...
            try:
                refactored_pickup_location = find_address_by_name(
                    self.pickup_location, self.trip.from_city.name
                )
        ...
        if self.dropoff_location and self.dropoff_location.strip():
            # Проверяем, соответствует ли адрес уже нашему формату
            try:
                refactored_dropoff_location = find_address_by_name(
                    self.dropoff_location, self.trip.to_city.name
                )
        ...
        if hasattr(self, "_seats_to_validate"):
            ...
            for seat in seats_to_validate:
                ...
                if seat.vehicle != self.trip.vehicle:
                    raise ValidationError(
                        f"Место {seat} не соответствует транспортному средству рейса"
                    )
        ...
        if self.pk:  # для существующих экземпляров
            for trip_seat in self.trip_seats.all():
                ...
                if trip_seat.seat.vehicle != self.trip.vehicle:
                    raise ValidationError(
                        f"Место {trip_seat.seat} не соответствует транспортному средству рейса"
                    )

                # Проверка, не занято ли место в другом бронировании
                if trip_seat.is_booked:
                    current_booking_seats = Booking.objects.get(
                        pk=self.pk
                    ).trip_seats.all()
                    if trip_seat not in current_booking_seats:
                        raise ValidationError(
                            f"Место {trip_seat.seat} уже забронировано"
                        )
