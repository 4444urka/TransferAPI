...
class BookingPermissionsTest(RequestsGetPatchedAPITestCase):
    ...
    def setUp(self):
        ...
        self.manager_user = User.objects.create_user(
            phone_number="+79222222222", password="manager123"
        )
        ...
        self.regular_user1 = User.objects.create_user(
            phone_number="+79333333333", password="user123"
        )

        self.regular_user2 = User.objects.create_user(
            phone_number="+79444444444", password="user123"
        )

        # Создаем водителя и добавляем его в группу Водитель
        self.driver = User.objects.create_user(
            phone_number="+79555555555", password="driverpass"
        )
        ...
        self.trip_seat1 = TripSeat.objects.filter(
            trip=self.trip, seat__seat_number=1
        ).first()
        self.trip_seat2 = TripSeat.objects.filter(
            trip=self.trip, seat__seat_number=2
        ).first()
    ...
    def test_create_booking_as_regular_user(self):
        ...
        with (
            patch(
                "utils.address.find_address_by_name", return_value="ул. Тестовая, 10"
            ),
            patch(
                "apps.booking.services.BookingService.create_booking"
            ...
            def create_booking_side_effect(validated_data, initial_data):
                ...
                for seat_number in seat_numbers:
                    trip_seat = TripSeat.objects.get(
                        trip=trip, seat__seat_number=seat_number
                    )
            ...
            available_trip_seat = TripSeat.objects.filter(
                trip=self.trip, is_booked=False
            ).first()
...
class BookingFilterTest(RequestsGetPatchedAPITestCase):
    ...
    def setUp(self):
        ...
        vehicle = Vehicle.objects.create(
            vehicle_type="bus", license_plate="А123АА", total_seats=40, is_comfort=True
        )
        ...
        self.trip2 = Trip.objects.create(
            ...
            economy_seat_price=Decimal("1200.00"),
            comfort_seat_price=Decimal("1200.00"),
        )
    ...
    def test_filter_bookings_by_active_status(self):
        ...
        with patch("utils.address.find_address_by_name") as mock_find_address:
            ...
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(
                len(response.data["results"]), 2, "Должно быть два бронирования в базе"
            )
            ...
            self.assertIn(
                ...
                all_booking_ids,
                "Активное бронирование должно быть в результатах",
            )
            self.assertIn(
                self.canceled_booking.id,
                all_booking_ids,
                "Отмененное бронирование должно быть в результатах",
            )
            ...
            self.assertGreaterEqual(
                ...
                1,
                "Должно быть хотя бы одно активное бронирование",
            )
            ...
            self.assertGreaterEqual(
                len(response.data["results"]),
                1,
                "Должно быть хотя бы одно неактивное бронирование",
            )

            # Получаем ID всех неактивных бронирований в ответе
            inactive_booking_ids = [
                booking["id"] for booking in response.data["results"]
            ]
...
class BookingAPITest(RequestsGetPatchedAPITestCase):
    ...
    @patch("apps.booking.services.find_address_by_name")
    @patch("apps.booking.models.find_address_by_name")
    @patch("apps.booking.serializers.find_address_by_name", create=True)
    def test_create_booking(
        self,
        mock_find_address_serializer,
        mock_find_address_model,
        mock_find_address_service,
    ):
        """Тест создания бронирования"""
        # Настраиваем мок
        mock_find_address_serializer.return_value = "ул. Арбат, 1"
        mock_find_address_model.return_value = "ул. Арбат, 1"
        mock_find_address_service.return_value = "ул. Арбат, 1"
