...
class BookingViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                "trip",
                openapi.IN_QUERY,
                description="Фильтр по ID поездки",
            ...
            openapi.Parameter(
                "ordering",
                openapi.IN_QUERY,
                description="Сортировка (например, -booking_datetime)",
                type=openapi.TYPE_STRING,
            ),
        ],
        tags=["Бронирования"],
    ...
    def get_queryset(self):
        ...
        queryset = Booking.objects.select_related(
            "user", "trip", "payment"
        ).prefetch_related("trip_seats__seat")
        ...
        if (
            not self.request.user.has_perm("booking.can_view_all_booking")
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "trip_id": openapi.Schema(
                    type=openapi.TYPE_INTEGER, description="ID поездки"
                ),
                ...
                "pickup_location": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Адрес посадки"
                ),
                "dropoff_location": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Адрес высадки"
                ),
            },
        ),
        tags=["Бронирования"],
    ...
    def cancel(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"message": "Бронирование успешно отменено"}, status=status.HTTP_200_OK
            )
        except Exception as e:
            logger.error(f"Error canceling booking: {e}")
            return Response(
                {"error": f"Ошибка при отмене бронирования: {str(e)}"},
...
def get_trip_seats(request):
    ...
    if trip_id:
        try:
            ...
            trip_seats = TripSeat.objects.filter(trip=trip).order_by(
                "seat__seat_number"
            )

            # Формируем список для JSON
            data["seats"] = [{"id": ts.pk, "text": str(ts)} for ts in trip_seats]
        except ValueError:
