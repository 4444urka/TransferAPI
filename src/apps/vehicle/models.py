...
class Vehicle(models.Model):
    vehicle_type = models.CharField(
        max_length=30, choices=VEHICLE_TYPE_CHOICES, verbose_name="Тип транспорта"
    )
    ...
    rows = models.IntegerField(null=True, blank=True, verbose_name="Количество рядов")
    seats_per_row = models.IntegerField(
        null=True, blank=True, verbose_name="Количество мест в ряду"
    )
    is_comfort = models.BooleanField(default=False, verbose_name="Повышенный комфорт")
    air_conditioning = models.BooleanField(default=True, verbose_name="Кондиционер")
    allows_pets = models.BooleanField(default=False, verbose_name="Можно с животными")
    is_right_hand_drive = models.BooleanField(default=False, verbose_name="Правый руль")
    ...
    def clean(self):
        ...
        if not self.total_seats:
            raise ValidationError(
                "Необходимо указать либо количество мест (total_seats), либо количество рядов и мест в ряду (rows, seats_per_row)"
            )
        ...
        if self.vehicle_type == "premium_car" and not self.is_comfort:
            raise ValidationError(
                "Премиум автомобиль должен иметь повышенный уровень комфорта"
            )
